...
df.all.area <- as.matrix(df[,c('Mean.Max.Temp','Total.Precip','Tourism.PC','Fire.Area')])
df.all.perc <- as.matrix(df[,c('Mean.Max.Temp','Total.Precip','Tourism.PC','Fire.Area.Perc')])
...
all.model.area <- lm(Fire.Area~
                       ...
                       X5+X6+X7+X8+X9+
                       Mean.Max.Temp+Total.Precip+Tourism.PC,df)

proj.all.model.area <- lm(Fire.Area~
                            Mean.Max.Temp+Total.Precip+Tourism.PC,df.proj.all.area)
...
proj.all.model.perc <- lm(Fire.Area.Perc~
                            Mean.Max.Temp+Total.Precip+Tourism.PC,df.proj.all.perc)
...
stargazer(all.model.area, proj.all.model.area, all.model.perc, proj.all.model.perc,
          ...
          covariate.labels = c('Bulkley Nechako + Stikine','Capital','Cariboo','Central Kootenay','Central Okanagan','Columbia Shuswap','Comox + Strathacona',
                               ...
                               'May','June','July','August','September','Avg. Max Temp','Avg. Total Precipitation','Avg. Tourism Tax Per Capita'),
          notes.label = 'Significance levels',
          report = 'vcp*',
          style = 'default',
          out = 'output/NL_Wass-Little_Thesis_Regressions_All_variables_w_proj.html')
...
stargazer(all.model.area, proj.all.model.area, all.model.perc, proj.all.model.perc,
          ...
          omit = c('Bulkley.Nechako...Stikine','Capital','Cariboo','Central.Kootenay','Central.Okanagan','Columbia.Shuswap','Comox...Strathacona',
                   ...
                   'X2002','X2003','X2004','X2005','X2006','X2007','X2008','X2009','X2010','X2011','X2012','X2013','X2014','X2015','X2016','X2017','X2018','X2019','X2020','X2021',
                   'X5','X6','X7','X8','X9'),
          covariate.labels = c('Avg. Max Temp','Avg. Total Precipitation','Avg. Tourism Tax Per Capita'),
          notes.label = 'Significance levels',
          report = 'vcp*',
          style = 'default',
          out = 'output/NL_Wass-Little_Thesis_Regressions_w_proj.html')
